...
import { Plus, Calendar, Dog as DogIcon } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <div className="space-y-12">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black">My Pack</h1>
            <p className="font-bold text-muted-foreground">Manage your furry friends</p>
          </div>
          <div className="flex gap-4">
            <Button asChild className="playful-btn bg-playful-green hover:bg-playful-green/90 text-black">
              <Link to="/dogs/new"><Plus className="mr-2" /> Add Dog</Link>
            </Button>
        ...
        <section>
          <h2 className="text-2xl font-black mb-6 flex items-center gap-2">
            <DogIcon className="text-playful-blue" /> Your Dogs
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {MOCK_DOGS.map((dog) => (
              <div key={dog.id} className="playful-card p-6 space-y-4 overflow-hidden relative">
                ...
                <div className="space-y-1 text-sm font-bold">
                  ...
                  <p className="text-muted-foreground">Behavior: <span className="text-black capitalize">{dog.behavior}</span></p>
                </div>
                <Button variant="outline" className="w-full border-2 border-black font-bold">Edit Profile</Button>
              </div>
            ))}
          </div>
        </section>
                  ...
                  return (
                    <tr key={booking.id} className="border-b-2 border-black/5 last:border-0 hover:bg-muted/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full border-2 border-black text-xs uppercase ${

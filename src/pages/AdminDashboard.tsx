...
import { LayoutDashboard, Users, AlertTriangle, Utensils } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <div className="space-y-8">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-black">Management Hub</h1>
            <p className="font-bold text-muted-foreground">Daily operations overview</p>
          </div>
          <div className="bg-playful-blue text-white border-4 border-black px-4 py-2 rounded-xl font-black">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="playful-card p-6 bg-playful-yellow flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-bold text-sm">Total Dogs</p>
            </div>
          </div>
          <div className="playful-card p-6 bg-playful-green flex items-center gap-4">
            ...
            <div>
              <p className="font-black text-3xl">{arrivingToday.length}</p>
              <p className="font-bold text-sm">Arriving Today</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <section className="space-y-4">
            <h2 className="text-2xl font-black">Daily Roster</h2>
                ...
                return (
                  <div key={booking.id} className="playful-card p-4 flex items-center justify-between bg-white">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-playful-blue border-2 border-black rounded-lg flex items-center justify-center font-black text-white">
                        {dog?.name[0]}
                      </div>
                      <div>
                        <h4 className="font-black">{dog?.name}</h4>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-bold text-muted-foreground">Check-in</p>
                    </div>
                  </div>
                );
              })}
          ...
          <section className="space-y-4">
            <h2 className="text-2xl font-black text-playful-pink">Care Alerts</h2>
            <div className="space-y-4">
              {MOCK_DOGS.filter(d => d.behavior === 'aggressive' || d.behavior === 'reactive' || d.instructions).map(dog => (
                <div key={dog.id} className="playful-card p-5 border-l-8 border-l-playful-pink bg-playful-pink/5">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className="text-playful-pink shrink-0" />
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {dog.behavior === 'reactive' && <span className="bg-playful-pink text-white px-2 py-1 rounded-lg text-xs font-black">REACTIVE</span>}
                        <span className="bg-playful-blue/10 text-playful-blue px-2 py-1 rounded-lg text-xs font-black flex items-center gap-1">
                          <Utensils className="w-3 h-3" /> DIET: {dog.diet.slice(0, 20)}...
                        </span>
                      </div>
                      <p className="text-sm font-bold text-muted-foreground bg-white border-2 border-black/5 p-2 rounded-lg italic">
                        "{dog.instructions}"
                      </p>
                    </div>
                  </div>
